...
function OrderModal({
  ...
  async function submit(e: React.FormEvent) {
    ...
    const itemLines = cartItems
      .map((item) => `  • ${item.qty} x ${item.size} @ KES ${item.price.toLocaleString()} (TSh ${Math.round(item.price * 20.00).toLocaleString()}) = KES ${(item.price * item.qty).toLocaleString()} (TSh ${Math.round(item.price * item.qty * 20.00).toLocaleString()})`)
      .join('\n');
